...
// Modified install method without integration test
async install() {
    ...
    try {
        ...
        // Step 4: Setup Bitrix24 UI placements
        console.log('4️⃣ Setting up Bitrix24 UI placements...');
        const placementTest = await this.setupBitrix24Placement();

        // Step 5: Run integration test (optional)
        console.log('5️⃣ Running integration test...');
    ...
    async setupBitrix24Placement() {
    console.log('🖥️ Setting up Bitrix24 UI placement...');
    
    try {
        // Create placement in CRM section
        const placementResult = await this.callBitrix24Method('placement.bind', {
            PLACEMENT: 'CRM_LEAD_LIST_MENU', // Shows in Lead list menu
            HANDLER: `${this.baseUrl}/app`,
            TITLE: 'WhatsApp Connector',
            DESCRIPTION: 'Connect and manage WhatsApp messages'
        });
        
        console.log('✅ CRM Lead placement created:', placementResult);
        
        // Also add to Contact list menu
        const contactPlacementResult = await this.callBitrix24Method('placement.bind', {
        ...
        console.log('✅ CRM Contact placement created:', contactPlacementResult);
        
        // Add to main menu
        const mainMenuResult = await this.callBitrix24Method('placement.bind', {
            PLACEMENT: 'MENU',
            HANDLER: `${this.baseUrl}/app`,
            TITLE: 'WhatsApp',
        ...
        console.log('✅ Main menu placement created:', mainMenuResult);
        
        return {
            success: true,
            placements: {
                crmLead: placementResult,
                crmContact: contactPlacementResult,
                mainMenu: mainMenuResult
            }
        };
        
    } catch (error) {
        if (error.message.includes('already exists') || error.message.includes('already binded')) {
            console.log('ℹ️ Placements already exist (OK)');
            return { success: true, note: 'Placements already exist' };
        }
        
        console.error('❌ Error setting up placements:', error);
        return { success: false, error: error.message };
    }
}
